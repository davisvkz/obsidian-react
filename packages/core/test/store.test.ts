...
import { getFolderFiles, getSnapshot, subscribeFolderFiles } from "../src/store";

// ---------------------------------------------------------------------------
// Mock helpers
// ---------------------------------------------------------------------------

type ChangedCb = (file: { path: string }, data: string, cache: Record<string, unknown>) => void;
	...
	const app = {
		...
		vault: {
			...
			getFolderByPath: vi.fn((folder: string) => {
				...
				return {
					children: overrides?.folderChildren ?? [{ name: "index.md", path: "studied/index.md" }],
				};
			}),
			on: vi.fn(() => ({})),
		},
...
describe("store — lastCache: uses CachedMetadata from the 'changed' event", () => {
	it("getSnapshot reflects frontmatter from the 'changed' event even when getCache returns null", () => {
		...
		const unsub = subscribeFolderFiles(app as never, "studied", () => {}, null, true);
	...
	it("getFolderFiles reflects frontmatter from the 'changed' event even when getCache returns null", () => {
		const app = makeApp({
			folderChildren: [
				{ name: "index.md", path: "studied/index.md" },
				{ name: "session.md", path: "studied/Physics/records/session.md" },
			],
			getCache: () => null,
		});
		...
		app.fireChanged(
			...
			"---\ntype: record\ndurationMin: 90\n---\n",
			{ frontmatter: { durationMin: 90, type: "record" } },
		);
