...
describe("Nested to-dos (folder-per-todo)", () => {
	before(async () => {
	...
	it("ignores a subfolder without index.md (does not become a ghost node)", async () => {
	...
	it("lists a top-level to-do created in the vault", async () => {
	...
	it("renders subtasks recursively on the detail page", async () => {
	...
	it("reflects a `done` change made externally (home)", async () => {
	...
	it("clicking a subtask checkbox writes `done` to the file (round-trip)", async () => {
		await navigateToDetail("Buy");
		await waitForDetail();
		await waitForTodos(
	...
	it("deletes the top-level to-do (and the entire subtree disappears from the vault)", async () => {
