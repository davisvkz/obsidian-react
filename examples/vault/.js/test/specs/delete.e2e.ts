...
function readTodos() {
	return browser.executeObsidian(() => {
		...
		return deepQueryAll("input.mantine-Checkbox-input").map((el) => {
			...
			return { checked: inp.checked, label };
		});
	});
}
...
function navigateToDetail(label: string) {
	return browser.executeObsidian((_obs, nodeLabel) => {
		const roots: Element[] = [];
		const walk = (node: Document | ShadowRoot) => {
			roots.push(...Array.from(node.querySelectorAll(".mantine-Checkbox-root")));
			for (const el of Array.from(node.querySelectorAll("*"))) {
...
describe("Deletion reflects in the UI", () => {
	before(async () => {
	...
	beforeEach(async () => {
		...
		await waitForTodos((t) => t.some((x) => x.label === "Buy"), "to-do Buy did not appear on home");
	});
	...
	it("removes a nested node when deleted manually (vault)", async () => {
	...
	it("removes a nested node when the trash icon is clicked (UI)", async () => {
		await navigateToDetail("Buy");
		await waitForDetail();
		await waitForTodos(
	...
	it("removes a top-level node when the trash icon is clicked (home)", async () => {
	...
	it("removes a node when only its index.md note is deleted (leaf)", async () => {
		await navigateToDetail("Buy");
		await waitForDetail();
		await waitForTodos(
			(t) => t.some((x) => x.label === "Milk"),
	...
	it("removes a subtree when the index.md of a parent node is deleted", async () => {
		await navigateToDetail("Buy");
		await waitForDetail();
		await waitForTodos(
			(t) => t.some((x) => x.label === "Bread"),
	...
	it("removes a nested node with local trash (.trash)", async () => {
		await navigateToDetail("Buy");
		await waitForDetail();
		await waitForTodos(
			(t) => t.some((x) => x.label === "Milk"),
